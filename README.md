...
rlusd wallet generate --chain xrpl --name my-xrpl --password s3cret --no-store-in-keychain
```
...
```bash
rlusd wallet keychain enable my-xrpl --password your-existing-password
```
...
Request test funds on supported test networks.
...
For XRPL testnet, the RLUSD flow is:
1. Use `rlusd faucet fund --chain xrpl` until the address is activated with XRP.
2. Create the RLUSD trust line:

```bash
rlusd xrpl trustline setup
```

3. If the account already has XRP, run the faucet command again. The CLI will direct you to the official RLUSD faucet:
